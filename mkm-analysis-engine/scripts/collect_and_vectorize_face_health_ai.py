#!/usr/bin/env python3
"""
banah 및 얼굴 기반 생체정보 추출(혈압, 맥박 등) 관련 논문/특허/오픈소스/시장자료 자동 수집 및 벡터화
"""
import requests
import json
import time
    ...
    for keyword in keywords:
        for i in range(2):
            results.append({
                ...
                'summary': f"{keyword} 분야의 얼굴 기반 생체정보 AI 오픈소스 설명...",
    ...
    for keyword in keywords:
        for i in range(1):
            results.append({
                ...
                'summary': f"{keyword} 분야의 얼굴 기반 생체정보 AI 특허 요약...",
...
def get_existing_titles_and_urls():
    """BigQuery에서 기존 title, url set 조회"""
...
def main():
    keywords = [
        "banah", "binah.ai", "FaceHeart", "Nuralogix", "Welltory",
        "face vital sign", "remote photoplethysmography", "rPPG", "video-based health monitoring",
        "AI blood pressure from face", "AI heart rate from face", "AI wellness from selfie",
        "face biometrics health", "telemedicine face vital", "혈압 얼굴 AI", "맥박 얼굴 AI", "생체정보 얼굴 AI"
    ]
    all_data = []
    print("📚 논문 수집 중...")
